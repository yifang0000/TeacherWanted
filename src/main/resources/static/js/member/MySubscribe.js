...
const app_subscirbe = Vue.createApp({          
  ...
  mounted() {
    ...
    this.getMemberActivitySubscribe();                            //
  },
  methods: {

    getMemberActivitySubscribe() {
      axios.get(this.url,{                       //promise 等後端回應
                                  
        activityId : this.activityId,
        teaId: this.teaId,
        activityType: this.activityType,
        currentNumeber: this.currentNumeber,
        ...
        activityDueTime: this.activityDueTime,
        ...
        subscribeActivity:[],
        this.imgSrc ="data:image/" + data.activityPhotoType +";base64," +
        data.activityPhoto,
...
$(document).ready(function () {
  ...
  axios.get("/calendarGet").then((res) => {
    for (let i = 0; i < res.data.length; i++) {
      ...
      $("#mainCalendar").fullCalendar("renderEvent", event);
    }
  });
