...
$(document).ready(function() {
    ...
    // 發送AJAX請求取得資料
    $.ajax({
      url: '/course_orders',  // 請將URL替換為實際的資料取得端點
      type: 'POST',
      dataType: 'json',
      success: function(response) {
        ...
        response.forEach(function(order) {
          table.row.add([
            ...
            order.discountPrice,
            order.orderStatus,
            '<button class="editor-delete">刪除</button>'  // 修改class屬性
          ]);
        });
        ...
        // 綁定刪除按鈕的點擊事件
        $('#courseOrderTable').on('click', '.editor-delete', function() {
          var row = $(this).closest('tr');
          var orderId = table.row(row).data()[0];  // 
  
          if (confirm('確定要刪除嗎？')) {
            // 發送刪除請求到後端 API
            fetch('../course_order/' + orderId, {  // 修改URL路徑
              method: 'DELETE'
            })
