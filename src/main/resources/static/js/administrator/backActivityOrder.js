          ...
          success: function(response) {
            ...
            response.forEach(function(order) {
              table.row.add([
                ...
                order.memName,
                order.memPhone,
                '<button class="editor-delete">刪除</button>'  // 修改為class屬性
              ]);
            });
            ...
            // 綁定刪除按鈕的點擊事件
            $('#activeOrderTable').on('click', '.editor-delete', function() {
              var row = $(this).closest('tr');
              var groupOrderDetailId = table.row(row).data()[0];  // 獲取groupOrderDetailId
      
              if (confirm('確定要刪除嗎？')) {
                // 發送刪除請求到後端 API
                fetch('../activeBackDelete/' + groupOrderDetailId, {  // 修改URL路徑
                  method: 'DELETE'
                })
